...
$getEmail = $mysqli->prepare('SELECT people_id, people_name, people_dp, people_email, people_topCode FROM people WHERE people_email=? AND people_pass=?') or die('Couldn\'t check the email');
...
if($countRows == 1){
	session_start();
	$getEmail->bind_result($id, $name, $dp, $emails,$topCode); // build cookie
